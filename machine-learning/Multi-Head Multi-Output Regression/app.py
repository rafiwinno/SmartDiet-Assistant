...
def map_meal_category(food_name: str) -> str:
    ...
    breakfast_keywords = [
        ...
        'breakfast', 'bagel', 'toast', 'bread',
        'bun', 'roll', 'egg', 'omelet',
        'omelette', 'scramble', 'pancake',
        'waffle', 'porridge', 'oat',
        'oatmeal', 'cereal', 'granola',
        'milk', 'yogurt', 'coffee',
        'tea', 'smoothie', 'banana',
        'apple', 'fruit bowl', 'jam',
        'margarine', 'butter'
    ]
...
food_df = food_df[
    food_df['Calories (kcal per 100g)'] < 500
]

food_df = food_df[
    food_df['Protein (g per 100g)'] > 5
].copy()
...
def calculate_tdee(
    ...
    if activity_level not in activity_map:
        ...
        raise ValueError(
            ...
            "activity_level harus salah satu dari: "
...
def predict_nutrition(user: NutritionInput):
    ...
    and user.target_weight >= user.weight_kg
    ):
        return {
        "error": (
            "Untuk weight_loss, "
            "target_weight harus lebih kecil dari weight_kg."
        )
    }
    ...
    and user.target_weight <= user.weight_kg
        ...
        "error": (
            "Untuk weight_gain, "
            "target_weight harus lebih besar dari weight_kg."
        )
    }
    ...
    and user.target_weight != user.weight_kg
        ...
        "error": (
            "Untuk maintain, "
            "target_weight harus sama dengan weight_kg."
        )
    }
    ...
    if isinstance(prediction_scaled, list):
        ...
        prediction_scaled = np.concatenate(
            ...
            [
                ...
                for output in prediction_scaled
            ],
        ...
        if prediction_scaled.ndim == 3:
            ...
            prediction_scaled.ndim == 2
            and prediction_scaled.shape == (4, 1)
        ):
    ...
    adjusted_protein = (
        adjusted_calories * protein_ratio / 4
    )

    adjusted_fat = (
        adjusted_calories * fat_ratio / 9
    )

    adjusted_carbs = (
        adjusted_calories * carbs_ratio / 4
    )
...
def apply_food_penalty(
    ...
    data['bonus_score'] = data.apply(
        ...
        if (
            row['meal_category'] == 'breakfast'
            and any(
                keyword in str(row['food']).lower()
...
def pick_meal_candidates(
    ...
    if meal_time == 'breakfast':
        ...
        main_candidates = (
            ...
            .head(min(top_n, len(main_candidates)))
        )
        ...
        snack_candidates = (
            ...
            snack_candidates
            .sort_values(by='final_score')
            .head(min(top_n, len(snack_candidates)))
        )
        ...
        if picked.empty:
            ...
            fallback = (
                ...
                fallback
                .sort_values(by='final_score')
                .head(min(sample_n, len(fallback)))
            )
