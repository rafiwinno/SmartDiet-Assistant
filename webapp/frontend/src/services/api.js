...
export async function login(email, password) {
  ...
  localStorage.setItem('user',      JSON.stringify({ id: res.data.user_id, name: res.data.user_name }))
  return res.data
}
...
export async function saveProfile(profileData) {
  const payload = {
    ...
    weight_kg:      parseFloat(profileData.weight),       // mapping
    height_cm:      parseFloat(profileData.height),       // mapping
    gender:         profileData.gender,
    activity_level: profileData.activityLevel,            // mapping
    goal:           profileData.goal,
    dietary:        profileData.dietary   || [],
    allergies:      profileData.allergies || [],
  }
  const res = await api.put('/user/profile', payload)
  return res.data  // { bmr, tdee, calorie_target, ... }
}
